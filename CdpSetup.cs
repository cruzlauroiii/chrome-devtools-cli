...
public partial class CdpCli
{
    ...
    private static bool DismissInfobar(bool Debug = false)
    {
        try
        {
            ...
            foreach (AutomationElement Window in Root.FindAll(TreeScope.Children, ChromeCondition))
            {
                // Find "Turn off in settings" button — only exists when infobar is showing
                var ButtonCondition = new PropertyCondition(AutomationElement.ControlTypeProperty, ControlType.Button);
                double InfobarY = -1;
                ...
                foreach (AutomationElement Button in Window.FindAll(TreeScope.Descendants, ButtonCondition))
                {
                    ...
                    if (Name == "Turn off in settings") InfobarY = Rect.Y;
                }
                if (InfobarY < 0) continue;
                // Now find the "Close" button in the same Y row as the infobar (within 20px)
                foreach (AutomationElement Button in Window.FindAll(TreeScope.Descendants, ButtonCondition))
                {
                    var Name = Button.Current.Name;
                    var Rect = Button.Current.BoundingRectangle;
                    ...
                    {
                        CloseBtn = Button;
                        if (Debug) Console.Error.WriteLine(string.Concat("Infobar Close at ", (int)Rect.X, ",", (int)Rect.Y, " rect=", Rect));
                    }
                }
                if (CloseBtn != null)
                {
                    // Try InvokePattern first (most reliable, no DPI issues)
                    try
                    {
                    ...
                    catch { /* fall through to mouse click */ }
        ...
        catch (Exception Ex) { if (Debug) Console.Error.WriteLine(string.Concat("DismissInfobar error: ", Ex.Message)); return false; }
    }
    ...
    private static void FocusChrome()
    {
        ...
        {
            SwitchToWindow(new IntPtr(Window.Current.NativeWindowHandle));
            Thread.Sleep(300); DismissInfobar();
